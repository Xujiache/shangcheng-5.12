...
async function main() {
  ...
  await test('real customer/order editors guard unfinished loads, release errors and support retry without blank saves', async () => {
    for (const name of ['customer', 'order']) {
      ...
      const editorPath =
        name === 'order'
          ? 'subpackages/orders/pages/order-edit/index.ts'
          : 'pages/customer-edit/index.ts'
      const { instance: p } = capture(editorPath, {
  ...
  await test('all registered pages use shared motion/feedback; native fixed positioning is not transformed', () => {
    ...
    for (const route of routes) {
      ...
      if (/^pages\//.test(route) && /wx\.redirectTo\(/.test(script)) {
        const target = script.match(/url:\s*'(\/subpackages\/[^']+)'/)
        assert(target && routes.includes(target[1].slice(1)), route)
        assert(template.includes('redirect'), route)
        continue
      }
