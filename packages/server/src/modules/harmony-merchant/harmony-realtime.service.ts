...
export class HarmonyRealtimeService implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  private async handle(socket: WebSocket, raw: RawData) {
    ...
    if (!(await this.isCurrentIdentity(state))) {
      this.clients.delete(socket)
      socket.close(4401, 'identity expired or revoked')
      return
    }
  ...
  private async authenticate(socket: WebSocket, token: string, requestId?: string) {
    try {
      ...
      if (payload._r || payload.scope || !payload.sub || typeof payload.exp !== 'number' || payload.exp * 1000 <= Date.now()) throw new Error('无效访问令牌')
      ...
      if (previous && (previous.userId !== user.id || previous.merchantId !== user.merchantId)) {
        throw new Error('请重新连接后切换账号')
      }
  ...
  private async isCurrentIdentity(state: HarmonyClientState): Promise<boolean> {
    ...
    try {
      ...
      if (!user || user.status !== 'active' || user.merchantId !== state.merchantId) return false
      const merchant = await this.prisma.merchant.findUnique({ where: { id: state.merchantId } })
