...
export class HarmonyPushService {
  ...
  async sendToMerchant(
    ...
    try {
      ...
      this.logger.warn('Harmony Push Kit 下发失败（已省略敏感详情）')
  ...
  private getCredentials(): HuaweiPushCredentials | null {
    ...
    try {
      ...
      if (account.project_id && account.project_id !== projectId) throw new Error('project mismatch')
      const keyId = String(
      ...
      if (privateKey && createPrivateKey(privateKey).asymmetricKeyType !== 'rsa') throw new Error('RSA key required')
      this.credentialsCache =
        projectId && keyId && subAccount && privateKey
          ? { projectId, keyId, subAccount, privateKey }
          : null
    } catch {
      this.logger.warn('读取 Harmony Push Kit 服务账号失败（已省略敏感详情）')
  ...
  private async sendBatch(
    ...
    if (!success) {
      this.logger.warn(
        `Harmony Push Kit 返回失败 status=${response.statusCode} code=${/^\d{1,12}$/.test(String(data.code)) ? data.code : 'unknown'}`,
      )
    }
    return success
  }
