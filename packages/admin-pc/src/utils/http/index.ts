...
axiosInstance.interceptors.response.use(
  (response: AxiosResponse<BaseResponse>) => {
    ...
    // 成功条件：code===0 (NestJS 标准) 或 code===200 (旧约定)
    if (code === 0 || code === ApiStatus.success) {
      if (!response.data.msg && text) response.data.msg = text
      return response
    }
    // 未登录/Token过期
    if (code === ApiStatus.unauthorized || code === 2001 || code === 2002) {
      handleUnauthorizedError(text)
    }
    throw createHttpError(text || $t('httpMsg.requestFailed'), code)
  },
  (error) => {
