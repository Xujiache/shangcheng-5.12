...
export class MenuProcessor {
   ...
   * 处理前端控制模式的菜单
   *
   * 角色匹配规则（使用 userStore.role 归一化后的角色）：
   * - 超管（role = 'super-admin'）：用 currentWorkspace 当作有效角色过滤
   * - 商家（role = 'merchant'，覆盖后端 factory / store / merchant 三态）：仅 /merchant/*
   * - 平台（role = 'platform'，覆盖后端 platform / admin）：仅 /platform/*
   * - guest：返回空数组，由守卫层处理（不会直接抛错让 500 卡死）
   */
  private async processFrontendMenu(): Promise<AppRouteRecord[]> {
    const userStore = useUserStore()
    const primaryRole = userStore.role
    const isSuper = primaryRole === 'super-admin'
    const isGuest = primaryRole === 'guest'

    const effectiveRoles: string[] = isSuper
    ...
    if (effectiveRoles.length > 0) {
      menuList = this.filterMenuByRoles(menuList, effectiveRoles)
    } else if (isGuest) {
      // 未知角色：不过滤、不返回菜单，由调用方处理跳转（避免 500 卡死）
      menuList = []
    }
  ...
  private validateMenuPaths(menuList: AppRouteRecord[], level = 1): void {
    menuList.forEach((route) => {
      ...
      route.children.forEach((child) => {
        ...
        // 跳过合法的绝对路径：外部链接、iframe 路由、分组下保持原 URL 的叶子路由
        if (this.isValidAbsolutePath(childPath, child)) return
   ...
   * 判断是否为合法的绝对路径
   */
  private isValidAbsolutePath(path: string, route?: AppRouteRecord): boolean {
    return (
      path.startsWith('http://') ||
      path.startsWith('https://') ||
      path.startsWith('/outside/iframe/') ||
      this.isAbsoluteLeafRoute(path, route)
    )
  }

  /**
   * Vue Router 允许子路由使用绝对路径。
   * 这里用于菜单分组：分组只影响左侧菜单层级，叶子路由仍保持 /merchant/* 或 /platform/* 原 URL。
   */
  private isAbsoluteLeafRoute(path: string, route?: AppRouteRecord): boolean {
    return !!path.startsWith('/') && !!route?.component && !route.children?.length
  }
