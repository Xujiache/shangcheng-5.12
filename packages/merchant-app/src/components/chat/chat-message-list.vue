...
const groupedMessages = computed<GroupedItem[]>(() => {
  ...
  props.messages.forEach((message, index) => {
    ...
    if (index === 0 || !previousTime || timestamp - previousTime >= 10 * 60 * 1000) {
      result.push({ kind: 'time', id: `time-${message.id}`, label: formatMessageTime(message.createdAt) })
    }
...
function measureViewport() {
  nextTick(() => {
    try {
      ...
      query.select('.chat-message-list').boundingClientRect((rect: any) => {
...
function scrollToMessage(messageId: string, animated = true) {
  ...
  nextTick(() => {
    ...
    if (!animated) setTimeout(() => { scrollWithAnimation.value = true }, 80)
  })
}
  ...
  >
    <view class="message-list-inner">
        ...
        >
          <view class="avatar-wrap">
            <image
              ...
              mode="aspectFill"
            />
            <view v-else class="avatar-fallback">{{ item.message.sender === 'merchant' ? shopName.slice(0, 1) : userName.slice(0, 1) || '客' }}</view>
          </view>
          ...
          <view class="bubble-column">
            ...
            <view v-else class="text-bubble"><text selectable>{{ messageText(item.message) }}</text></view>

            <view v-if="item.message.sender === 'merchant' && item.message.id === lastMerchantId" class="send-state">
              <text v-if="item.message._status === 'sending'">发送中</text>
              <text v-else-if="item.message._status === 'failed'" class="send-failed" @click.stop="emit('retry', item.message)">发送失败，点击重试</text>
      ...
      <view v-if="!messages.length" class="empty-chat">
        <wd-status-tip  image="content" :tip="['还没有消息', '可以主动向客户发送问候'].filter(Boolean).join(' · ')" />
      </view>
    </view>
  </scroll-view>
</template>
...
.chat-message-list { flex: 1; min-height: 0; background: #f1f3f5; }
.message-list-inner { padding: 20rpx 24rpx 36rpx; }
.history-tip { padding: 8rpx 0 18rpx; color: #a2a7b0; font-size: 20rpx; text-align: center; }
.time-divider { width: fit-content; margin: 18rpx auto 24rpx; padding: 4rpx 13rpx; border-radius: 8rpx; color: #a2a7b0; background: rgba(255, 255, 255, .62); font-size: 20rpx; }
.system-message { width: fit-content; max-width: 82%; margin: 18rpx auto 26rpx; padding: 8rpx 16rpx; border-radius: 8rpx; color: var(--text-tertiary); background: rgba(255, 255, 255, .72); font-size: 21rpx; line-height: 1.5; text-align: center; }
.message-row { display: flex; align-items: flex-start; gap: 16rpx; margin-bottom: 28rpx; }
.message-row.merchant { flex-direction: row-reverse; }
.avatar-wrap { width: 70rpx; height: 70rpx; flex: 0 0 70rpx; }
.avatar-img, .avatar-fallback { width: 70rpx; height: 70rpx; border-radius: 12rpx; }
.avatar-fallback { display: flex; align-items: center; justify-content: center; background: #d9e1e7; color: #5f6b77; font-size: 25rpx; font-weight: 600; }
.merchant .avatar-fallback { background: #fff0eb; color: #ff4d2d; }
.bubble-column { max-width: 72%; display: flex; flex-direction: column; align-items: flex-start; }
.merchant .bubble-column { align-items: flex-end; }
.text-bubble { padding: 17rpx 21rpx; border-radius: 6rpx 18rpx 18rpx; background: var(--bg-card); color: #1f2329; font-size: 27rpx; line-height: 1.55; word-break: break-all; }
.merchant .text-bubble { border-radius: 18rpx 6rpx 18rpx 18rpx; background: #ff5b3d; color: #fff; }
.image-bubble { position: relative; overflow: hidden; flex-shrink: 0; border: 1rpx solid rgba(0, 0, 0, .06); border-radius: 14rpx; background: #e5e6eb; }
.image-bubble.image-landscape { width: 300rpx; height: 200rpx; }
.image-bubble.image-portrait { width: 210rpx; height: 300rpx; }
.image-bubble.image-square { width: 260rpx; height: 260rpx; }
.message-image { position: absolute; inset: 0; display: block; width: 100%; height: 100%; opacity: 0; transition: opacity .16s ease; }
.message-image.loaded { opacity: 1; }
.image-skeleton { position: absolute; inset: 0; background: linear-gradient(100deg, #e7e9ec 20%, #f5f6f7 42%, #e7e9ec 64%); background-size: 220% 100%; animation: shimmer 1.25s ease-in-out infinite; }
.image-failed { position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8rpx; color: var(--text-tertiary); background: #eef0f2; font-size: 21rpx; }
.send-state { margin-top: 8rpx; color: #a2a7b0; font-size: 19rpx; }
.send-failed { color: #f53f3f; }
.empty-chat { padding-top: 130rpx; }
