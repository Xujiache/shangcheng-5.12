...
function createSocket(token: string, role: 'user' | 'merchant'): InternalSock {
  ...
  async function connect() {
    ...
    io.on('connect', () => {
      ...
      io.emit('auth', { token: authToken, role })
    })
    io.on('disconnect', () => {
...
/** refresh token 轮换后原连接原地重新鉴权，保留页面已注册的全部事件处理器。 */
export function refreshChatSocketToken(token: string) {
  if (!_singleton || _singleton._token === token) return
  if (!token) {
    destroyChatSocket()
    return
  }
  _singleton._updateToken(token)
}
