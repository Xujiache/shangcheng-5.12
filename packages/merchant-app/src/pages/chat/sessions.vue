    ...
    <view class="page">
      ...
      <view class="toolbar">
        <view class="tabs">
          ...
          <view :class="['tab', { active: tab === 'unread' }]" @click="tab = 'unread'">
            未读
            <text v-if="unreadTotal" class="tab-count">{{
      ...
      <view v-else class="state-wrap">
        <wd-status-tip
          ...
          :tip="
            [
              keyword ? '没有找到相关会话' : tab === 'unread' ? '没有未读消息' : '暂无客户消息',
              keyword ? '换个关键词试试' : '客户发来咨询后会显示在这里',
            ]
              .filter(Boolean)
              .join(' · ')
          "
...
.session-row + .session-row .session-main {
...
.skeleton {
  background: linear-gradient(90deg, #f1f2f4 25%, #f8f8f9 37%, #f1f2f4 63%);
