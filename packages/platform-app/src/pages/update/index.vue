...
function fallbackRoute() {
  ...
  try {
    return uni.getStorageSync('jiujiu_admin_token') ||
      uni.getStorageSync('jiujiu_admin_refresh_token')
...
function startDownload() {
  ...
  try {
    ...
    downloadTask = plus.downloader.createDownload(
      ...
      {
        method: 'GET',
        filename: `_downloads/jingwei-platform-${release.versionCode}.apk`,
        timeout: 120,
        retry: 3,
      },
    ...
    <view class="page">
      ...
      <GlassSurface class="card" variant="card" effect="auto">
        <view v-if="stage === 'loading'" class="center-state"
          ><view class="spinner" /><text>正在获取版本信息…</text></view
        >
        <template v-else-if="context && latest">
          <view class="version-row">
            <view class="version-block"
              ...
              ><text class="build">Build {{ context.runtime.versionCode }}</text></view
            >
            ...
            <view class="version-block right"
              ...
              ><text class="build">Build {{ latest.versionCode }}</text></view
            >
          </view>
          <view class="meta-row"
            ...
            ><text v-if="latest.force" class="force-tag">强制更新</text></view
          >
          <view class="notes"
            ><text class="notes-title">本次更新</text
            ><text class="notes-content">{{
              latest.changelog || '功能优化与稳定性提升'
            }}</text></view
          >
        ...
        <view v-else class="center-state"
          ><text class="error-title">版本信息获取失败</text><text>{{ errorMessage }}</text
...
.top-glow {
  ...
  height: 700rpx;
  border-radius: 50%;
  background: linear-gradient(135deg, #26334d, #172033 58%, #0e1421);
}
.close {
  position: absolute;
  z-index: 5;
...
.rocket {
  ...
  justify-content: center;
  border-radius: 34rpx;
  background: rgba(255, 77, 45, 0.85);
