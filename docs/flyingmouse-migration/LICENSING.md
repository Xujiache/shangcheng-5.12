...
- FFmpeg、LibreOffice、Poppler、Tesseract、Pandoc、qpdf、Node/Python 依赖须依据最终 worker 镜像生成 SBOM 与实际许可证清单。2026-09-26 用户要求先配置服务器，生产仅开启已验证的十四组格式及一项图片合成操作；源码授权文件尚未归档，第三方许可检查和真实格式质量验收也未完成，不能把本次技术部署视为许可审核通过。继续扩大格式白名单前须完成对应质量验收。
