...
function scan(source) {
  ...
  const localized = () =>
    callStack.some(
      (name) => name === 'I18n.text' || name === 'I18n.current' || name === 'I18n.contract',
    )
...
console.log(
  `Native i18n audit: ${total} unlocalized literal(s) remain in ${report.length} file(s).`,
)
for (const item of report.slice(0, 20)) {
  const examples = item.findings
    .slice(0, 2)
    .map((entry) => `${entry.line}:${entry.value}`)
    .join(' | ')
