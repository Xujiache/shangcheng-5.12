...
gh release upload v0.3.2 'dist\FlyingMouse Format-Setup-0.3.2-win7-x64.exe' --repo flyingmouse-format
```
...
```powershell
$existing=gh release view v0.3.2 --repo flyingmouse-format --json body --jq .body
$legacy=@(
...
gh release edit v0.3.2 --repo flyingmouse-format --notes ($existing.TrimEnd()+"`n`n"+$legacy)
```

- [ ] **Step 5: Read back and compare the remote asset**

```powershell
gh api repos/flyingmouse-format/releases/tags/v0.3.2 --jq '{tag_name:.tag_name,assets:[.assets[]|{name:.name,size:.size,digest:.digest,state:.state}]}'
git rev-parse HEAD
